...
var (
	tplsnake = `<!DOCTYPE html>
	<html lang="en">

	<head>
		<meta charset="UTF-8">
		<title>CodePen - React Game: Snake</title>
		<style>
		body, html {
			height: 100%;
			margin: 0;
	...
	<body>
		...
		<script>
		function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; } // Control with the arrow keys
...
class App extends React.Component {
    ...
    updateSnake() {
        ...
        if (newHead[0] > 390 || newHead[0] < 0 || newHead[1] > 320 || newHead[1] < 30) { // Enable this is you want the wall collusion rule
            // this.startStop(true);
            ...
            } else { currentSnake.shift(); if (this.state.active) { this.setState({ snake: currentSnake }); } }
        }
    }
    componentDidMount() {
        this.swapClass();
        document.addEventListener("keydown", this.handleKeys, false);
        if (this.state.active) { this.startStop(false); }
    }
    componentDidUpdate(prevProps, prevState) { // When the state changes, check if we've reached a % 5 milestone
        // Run speedUp once, but not again until next time (state updates each time snake moves)
        let score = this.state.score;
        if (score % 3 == 0 && score > 0 && score != prevState.score) { this.speedUp(); }
        document.addEventListener("keydown", this.handleKeys, false);
    }
    ...
    render() {
        ...
        return React.createElement(React.Fragment, null, React.createElement(Menu, { active: this.state.active }),
...
// GameSnake 贪吃蛇
func GameSnake(c *gin.Context) {
	c.Header("Content-Type", "text/html")
	c.Status(http.StatusOK)
	render.WriteString(c.Writer, tplsnake, nil)
}
