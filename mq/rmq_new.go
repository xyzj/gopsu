...
// CreateConsumer 初始化消费者实例
func CreateConsumer(name, connstr, queuename string, logger *gopsu.MxLog, durable, autodel, debug bool) *Session {
	sessn := &Session{
		logger:        logger,
		name:          name,
		connStr:       connstr,
...
// CreateProducer 初始化生产者实例
func CreateProducer(name, connstr string, logger *gopsu.MxLog, debug bool) *Session {
	sessn := &Session{
		logger:  logger,
		name:    name,
		connStr: connstr,
